...
const char * const BITCOIN_CONF_FILENAME = "trumprocket.conf";
...
#else
    const char* pszModule = "trumprocket";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TrumpRocket";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/TrumpRocket";
#else
    // Unix
    return pathRet / ".trumprocket";
#endif
#endif
}
